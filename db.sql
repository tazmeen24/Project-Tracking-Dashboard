...
-- ============================================================================
-- Clean, organized schema with no duplicates
-- ============================================================================
...
CREATE TABLE projects (
    ...
    project_category VARCHAR(50) NOT NULL CHECK (project_category IN ('sponsored', 'non-sponsored')),
    project_type VARCHAR(50) NOT NULL CHECK (project_type IN ('PFMS', 'NON-PFMS', 'contract-research')),
    pfms_id TEXT,
    CONSTRAINT chk_project_category_type_relation CHECK (
    ...
    CONSTRAINT chk_pfms_requires_identifier CHECK (
        NOT (project_category = 'sponsored' AND project_type = 'PFMS') 
        OR pfms_id IS NOT NULL
    )
);

CREATE INDEX IF NOT EXISTS idx_projects_pi ON projects(project_category);
...
-- Budget Allocation (Planned allocations)
...
-- Manpower Allocation Breakdown (planned at allocation time)
...
-- Funds Received (with budget head tagging)
CREATE TABLE funds_received (
    fund_id SERIAL PRIMARY KEY,
    project_id INT REFERENCES projects (project_id) ON DELETE CASCADE,
    installment_id INT REFERENCES project_installments(installment_id) ON DELETE CASCADE,
    head TEXT CHECK (
        head IN (
            'manpower',
            'equipment',
...
CREATE TABLE utilization_certificates (
    ...
    grant_general NUMERIC(14,2) DEFAULT 0,
    grant_salary NUMERIC(14,2) DEFAULT 0,
    grant_capital_assets NUMERIC(14,2) DEFAULT 0,
    expenditure_recurring NUMERIC(14,2) DEFAULT 0,
    expenditure_non_recurring NUMERIC(14,2) DEFAULT 0,
...
-- Compare Planned vs Actual for Manpower
...
FROM
    projects p
    LEFT JOIN budget_allocation ba ON p.project_id = ba.project_id AND ba.head = 'manpower'
    LEFT JOIN manpower_allocation_breakdown mab ON ba.allocation_id = mab.allocation_id
    LEFT JOIN manpower m ON p.project_id = m.project_id AND m.role = mab.role
GROUP BY
    p.project_id,
    p.title,
    mab.role,
...
-- Compare Planned vs Actual for Equipment
...
FROM
    projects p
    LEFT JOIN budget_allocation ba ON p.project_id = ba.project_id AND ba.head = 'equipment'
    LEFT JOIN equipment_allocation_breakdown eab ON ba.allocation_id = eab.allocation_id
    LEFT JOIN equipment e ON p.project_id = e.project_id AND e.name = eab.item_name
GROUP BY
    p.project_id,
    p.title,
    eab.item_name,
...
FROM
    projects p
    CROSS JOIN heads h
    LEFT JOIN budget_allocation ba ON p.project_id = ba.project_id AND ba.head = h.head
    LEFT JOIN funds_received fr ON p.project_id = fr.project_id AND fr.head = h.head
    LEFT JOIN manpower m ON p.project_id = m.project_id AND h.head = 'manpower'
    LEFT JOIN equipment e ON p.project_id = e.project_id AND h.head = 'equipment'
    LEFT JOIN budget_expenditure be ON p.project_id = be.project_id AND be.head = h.head
GROUP BY
    p.project_id,
    p.title,
    h.head,
...
SELECT 
    ...
    fr.fund_id, 
    fr.head, 
    SUM(COALESCE(mfb.total_amount, efb.total_amount, fr.amount)) AS detailed_fund_amount
FROM
    projects p
...
-- Verify funds exist in FY 2025-26
SELECT 
    fund_id,
    head,
    amount,
...
FROM funds_received 
WHERE project_id = 17 
  AND date_received BETWEEN '2025-04-01' AND '2026-03-31'
ORDER BY date_received;

-- Check if budget allocation exists
SELECT * FROM budget_allocation WHERE project_id = 17;

-- Check if project exists
SELECT * FROM projects WHERE project_id = 17;