...
import { X, Check, ChevronRight, ChevronLeft, Plus, Trash2, AlertCircle } from 'lucide-react';
...
const ProjectForm = ({ isOpen, onClose, onSuccess, editProject = null }) => {
  const { fundingAgencies, technicalGroups, refreshFundingAgencies, refreshTechnicalGroups } = useProject();
  ...
  const handleChange = (field, value) => {
    ...
    if (field === 'project_category') {
      if (value === 'sponsored') {
        ...
        setFormData(prev => ({ ...prev, project_type: 'contract-research', PFMS_id: '' }));
      }
    }
  ...
  const addManpowerRow = () => {
    setFormData(prev => ({
      ...prev,
      manpower_breakdown: [...prev.manpower_breakdown, {
        role: '',
        salary_per_month: 0,
        months: 1,
        num_personnel: 1,
  ...
  const addEquipmentRow = () => {
    setFormData(prev => ({
      ...prev,
      equipment_breakdown: [...prev.equipment_breakdown, {
        item_name: '',
        quantity: 1,
        unit_cost: 0,
      }]
  ...
  const getTotalManpowerAllocation = () => {
    return formData.manpower_breakdown.reduce((sum, row) => sum + calculateManpowerTotal(row), 0);
  };

  const getTotalEquipmentAllocation = () => {
    return formData.equipment_breakdown.reduce((sum, row) => sum + calculateEquipmentTotal(row), 0);
  };

  const getTotalBudget = () => {
    return getTotalManpowerAllocation() +
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (!formData.technical_group_id) newErrors.technical_group_id = 'Group is required';
  ...
  const handleSubmit = async () => {
    ...
    try {
      const projectData = {
        ...
        // Breakdowns
        manpower_breakdown: formData.manpower_breakdown,
        equipment_breakdown: formData.equipment_breakdown,
      };
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return <Step1ProjectMetadata formData={formData} errors={errors} handleChange={handleChange} fundingAgencies={fundingAgencies} technicalGroups={technicalGroups} />;
      case 2:
        return <Step2FundingAgency formData={formData} errors={errors} handleChange={handleChange} />;
      case 3:
        return <Step3Investigators formData={formData} errors={errors} handleChange={handleChange} />;
      case 4:
        return <Step4Timeline formData={formData} errors={errors} handleChange={handleChange} />;
      case 5:
        return <Step5BudgetSetup 
          formData={formData} 
          errors={errors} 
          ...
          {STEPS.map((step, index) => (
            <React.Fragment key={step.id}>
              <div className="flex flex-col items-center flex-1">
                ...
                <span className={`mt-2 text-xs font-medium ${currentStep >= step.id ? 'text-slate-900' : 'text-slate-400'}`}>
                  {step.title}
                </span>
              </div>
...
// Step Components
const Step1ProjectMetadata = ({ formData, errors, handleChange, fundingAgencies, technicalGroups }) => (
  <div className="space-y-6">
    ...
    {formData.project_category === 'sponsored' && formData.project_type === 'PFMS' && (
      <Input
        label="PFMS ID"
    ...
    <div className="grid grid-cols-2 gap-4">
      <div>
        ...
        {errors.technical_group_id && <p className="mt-1 text-sm text-red-600">{errors.technical_group_id}</p>}
      </div>
      ...
      <div>
        ...
        {errors.funding_agency_id && <p className="mt-1 text-sm text-red-600">{errors.funding_agency_id}</p>}
      </div>
    </div>
  </div>
);
  ...
  <div className="space-y-6">
    <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-xl mb-6">
      <p className="text-sm text-blue-800">
        <strong>Funding Agency Details:</strong> Provide detailed information about the funding agency for this project.
      </p>
    </div>
...
const Step3Investigators = ({ formData, errors, handleChange }) => (
  <div className="space-y-6">
    <div className="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded-xl">
      <h3 className="font-semibold text-indigo-900 mb-2">Principal Investigator</h3>
      <p className="text-sm text-indigo-800">Primary researcher responsible for the project.</p>
    </div>
    ...
    <div className="border-t border-slate-200 pt-6">
      <div className="bg-purple-50 border-l-4 border-purple-500 p-4 rounded-xl mb-4">
        <h3 className="font-semibold text-purple-900 mb-2">Co-Investigator (Optional)</h3>
        <p className="text-sm text-purple-800">Secondary researcher assisting with the project.</p>
      </div>
  ...
  <div className="space-y-6">
    <div className="bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded-xl">
      <p className="text-sm text-emerald-800">
        <strong>Project Duration:</strong> Specify the start and expected end dates for this project.
      </p>
    </div>
      ...
      <div className="bg-slate-50 p-4 rounded-xl">
        <p className="text-sm text-slate-700">
          <strong>Duration:</strong> {
            Math.ceil((new Date(formData.end_date) - new Date(formData.start_date)) / (1000 * 60 * 60 * 24 * 30))
          } months
        </p>
      </div>
    )}
...
const Step5BudgetSetup = ({ formData, errors, handleChange, manpower, equipment, totalBudget }) => (
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-600">Personnel costs with breakdown</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-right py-2 px-2">Salary/Month</th>
                <th className="text-right py-2 px-2">Months</th>
                <th className="text-right py-2 px-2">Personnel</th>
                ...
                <tr key={index} className="border-b border-slate-100">
                  <td className="py-2 px-2">
                    <input
                      type="text"
                      value={row.role}
                      onChange={(e) => manpower.update(index, 'role', e.target.value)}
                  ...
                  <td className="py-2 px-2">
                    <input
                      ...
                      onChange={(e) => manpower.update(index, 'salary_per_month', parseFloat(e.target.value) || 0)}
                  ...
                  <td className="py-2 px-2">
                    <input
                      type="number"
                      value={row.months}
                      onChange={(e) => manpower.update(index, 'months', parseInt(e.target.value) || 1)}
                      className="w-full px-2 py-1 border border-slate-200 rounded text-right"
                    />
                  </td>
                  <td className="py-2 px-2">
                    <input
                      ...
                      onChange={(e) => manpower.update(index, 'num_personnel', parseInt(e.target.value) || 1)}
                      className="w-full px-2 py-1 border border-slate-200 rounded text-right"
                    />
                  </td>
                  <td className="py-2 px-2 text-right font-semibold">
                    ₹{manpower.calculateTotal(row).toLocaleString('en-IN')}
                  </td>
                  <td className="py-2 px-2">
                    <button
      ...
      <div className="bg-blue-50 p-3 rounded-lg">
        <p className="text-sm font-semibold text-blue-900">
          Total Manpower Allocation: ₹{manpower.totalAllocation.toLocaleString('en-IN')}
        </p>
      </div>
    </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-600">Equipment and instruments with breakdown</p>
        </div>
                ...
                <tr key={index} className="border-b border-slate-100">
                  <td className="py-2 px-2">
                    <input
                      type="text"
                      value={row.item_name}
                      onChange={(e) => equipment.update(index, 'item_name', e.target.value)}
                  ...
                  <td className="py-2 px-2">
                    <input
                      type="number"
                      value={row.quantity}
                      onChange={(e) => equipment.update(index, 'quantity', parseInt(e.target.value) || 1)}
                      className="w-full px-2 py-1 border border-slate-200 rounded text-right"
                    />
                  </td>
                  <td className="py-2 px-2">
                    <input
                      type="number"
                      value={row.unit_cost}
                      onChange={(e) => equipment.update(index, 'unit_cost', parseFloat(e.target.value) || 0)}
                      className="w-full px-2 py-1 border border-slate-200 rounded text-right"
                    />
                  </td>
                  <td className="py-2 px-2 text-right font-semibold">
      ...
      <div className="bg-blue-50 p-3 rounded-lg">
        <p className="text-sm font-semibold text-blue-900">
          Total Equipment Allocation: ₹{equipment.totalAllocation.toLocaleString('en-IN')}
        </p>
      </div>
    </div>
