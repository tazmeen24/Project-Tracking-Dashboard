...
const FinancialSummaryPage = () => {
  ...
  const generateYearOptions = () => {
    ...
    const startYear = 2020; // or fetch from your earliest project data
  ...
  const getUtilizationColor = (percentage) => {
    if (percentage < 80)
      return "bg-green-100/80 dark:bg-green-950/40 text-green-800 dark:text-green-300 border border-green-200/50 dark:border-green-900/50";
    if (percentage < 100)
      return "bg-yellow-100/80 dark:bg-yellow-950/40 text-yellow-800 dark:text-yellow-300 border border-yellow-200/50 dark:border-yellow-900/50";
  ...
  const ProjectBudgetHeadDetailTable = ({
    data,
    formatCurrency,
    getBalanceColor,
  }) => {
    ...
    const renderBreakdown = (breakdown) => {
      ...
      return (
        <tr>
          <td
            colSpan="8"
            className="bg-slate-50/80 dark:bg-slate-900/30 px-6 py-4"
          >
            <div className="ml-10">
                  ...
                  <tbody>
                    {breakdown.map((item, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-sm text-right text-slate-800 dark:text-slate-200">
                          {formatCurrency(item.approved_budget)}
                        </td>
                        <td className="px-4 py-2 text-sm text-right text-slate-800 dark:text-slate-200">
                          {formatCurrency(item.funds_received)}
                        </td>
                        <td className="px-4 py-2 text-sm text-right text-slate-800 dark:text-slate-200">
              ...
              return (
                <React.Fragment key={id}>
                  <tr className="hover:bg-slate-50/80 dark:hover:bg-slate-700/30">
                    ...
                    <td className="px-6 py-4 text-sm text-right text-slate-800 dark:text-slate-200">
                      {formatCurrency(item.approved_budget)}
                    </td>
                    <td className="px-6 py-4 text-sm text-right text-slate-800 dark:text-slate-200">
                      {formatCurrency(item.funds_received)}
                    ...
                    <td
                      className={`px-6 py-4 text-sm text-right font-medium ${getBalanceColor(
                        item.budget_balance
                      )}`}
                    >
                      {formatCurrency(item.budget_balance)}
                      ...
                      <span
                        className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          item.utilization_percentage < 80
                            ...
                            ? "bg-yellow-100/80 dark:bg-yellow-950/40 text-yellow-800 dark:text-yellow-300 border border-yellow-200/50 dark:border-yellow-900/50"
          ...
          <tfoot className="bg-slate-100/80 dark:bg-slate-800/50 font-bold">
            <tr>
              ...
              {[
                ...
                <td
                  key={field}
                  className="px-6 py-4 text-sm text-right text-slate-900 dark:text-white"
                >
                  {formatCurrency(
                    sortedData.reduce((s, i) => s + (i[field] || 0), 0)
                  )}
                </td>
              ))}
              <td className="px-6 py-4 text-sm text-right text-slate-900 dark:text-white">
                {(() => {
                  const totalBudget = sortedData.reduce(
                    (s, i) => s + (i.approved_budget || 0),
                    0
                  );
                  const totalExp = sortedData.reduce(
                    (s, i) => s + (i.expenditure || 0),
                    0
                  );
                  return totalBudget > 0
                    ? ((totalExp / totalBudget) * 100).toFixed(1) + "%"
                    : "0.0%";
                })()}
              </td>
              <td></td>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-300 mt-2">
          Comprehensive financial overview with budget & fund tracking
        </p>
      </div>
      ...
      <div className="bg-white/80 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div>
            ...
            >
              ...
              <option value="project_budget_head_detail">
                Project Budget Head Detail
              </option>
            </select>
          </div>
          ...
          {dateFilterMode === "financial_year" && (
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
              >
                {generateFinancialYearOptions().map((fy) => (
            ...
            <div className="mt-10 bg-blue-50/80 dark:bg-blue-950/30 backdrop-blur-sm border-2 border-blue-200/50 dark:border-blue-900/50 rounded-2xl p-10 text-center">
              <div className="max-w-lg mx-auto">
                ...
                <p className="text-blue-700 dark:text-blue-300 text-lg">
                  Please select a <strong>project</strong> from the dropdown
                  above to view detailed financial information broken down by
                  Manpower, Equipment, Travel, etc.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="mt-8 flex justify-end">
          <button
            ...
            disabled={
              viewMode === "project_budget_head_detail" && !projectFilter
            }
          ...
          {[
            {
              ...
              value: financialData.summary.total_projects,
              color: "blue",
            },
            {
              label: "Approved Budget",
              value: formatCurrency(
                financialData.summary.total_approved_budget
              ),
              color: "purple",
            },
            {
              label: "Funds Received",
              value: formatCurrency(financialData.summary.total_funds_received),
              color: "green",
            },
            {
              label: "Expenditure",
              value: formatCurrency(financialData.summary.total_expenditure),
              color: "orange",
            },
            {
              label: "Budget Balance",
              value: formatCurrency(financialData.summary.budget_balance),
              color: "blue",
              balance: true,
            },
            {
              label: "Funds Balance",
              value: formatCurrency(financialData.summary.funds_balance),
              color: "teal",
              balance: true,
            },
          ].map((card, i) => (
            <div
              key={i}
              className="bg-white/80 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-6"
            >
                ...
                <div>
                  ...
                  <p
                    className={`text-2xl font-bold mt-1 ${
                      card.balance
                        ? getBalanceColor(card.value.replace(/[^0-9.-]+/g, ""))
            ...
            {financialData?.pagination && financialData.data?.length > 0 && (
              <div className="px-6 py-4 border-t border-slate-200/50 dark:border-slate-700/50 flex justify-between items-center">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  ...
                  of {financialData.pagination.total_items} entries
                </p>
